...
export default function Product({
    ...
    // console.log(places)
    return (
        <article className='group w-full bg-slate-50 rounded-lg drop-shadow-md hover:drop-shadow-xl duration-300 flex overflow-clip'>
            <div className='w-full flex flex-col'>
                <div className='relative'>
                    {media.length > 1 ? (
                        media.slice(0, 1).map(media => (
                            <div key={media.id} className='w-full aspect-video overflow-hidden'>
                                <Image className='group-hover:rotate-1 group-hover:scale-105 duration-300 w-full h-full object-cover aspect-video min-w-[200px] min-h-[200px]' src={media.url} alt={media.url} width={media.width}
                                       height={media.height}/>
                            </div>
                        ))
                    ) : media.length === 1 ? (
